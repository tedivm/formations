...
var astar = function (columns, rows) {
...
astar.prototype.search = function (room, start, end, user_options) {
    ...
    var options = Object.assign({}, astar.defaults, options)
    ...
    var graph = new Graph(room, this.columns, this.rows, weight, scoring, diagonal)
    ...
    if (scoring.structure !== false) {
      ...
      if (structures.length > 0) {
        for (var structure of structures) {
          ...
          if (!structure.my && (structureType != STRUCTURE_WALL && structureType != STRUCTURE_ROAD)) {
 ...
 */
function Graph(room, columns, rows, weight, scoring, diagonal) {
  this.room = room
  this.columns = columns
  this.rows = rows
  this.weight = weight
...
Graph.prototype.getNode = function (x, y) {
  ...
  if (!this.grid[x][y]) {
    var calculatedWeight = this.weight(this.room, x, y)
    this.grid[x][y] = new GridNode(x, y, calculatedWeight);
    /*
